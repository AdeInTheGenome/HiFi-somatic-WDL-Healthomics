...
task prioritize_dmr_intogen {
    ...
    output {
        ...
        File promoter_file = sname + "_hg38_genes_promoters_dmrs_intogen.nCG50_summary.tsv.gz"
    }
...
task prioritize_small_variants {
    ...
    echo -e "CHROM\tPOS\tREF\tALT\tFORMAT\t~{sname}\t$(bcftools +split-vep ~{vep_annotated_vcf} -l | cut -f2 | tr '\n' '\t' | sed 's/\t$//g')" > ~{fname}
...
task report_sample {
    ...
    mv visualize_hifisomatic.html ~{sname}_summary_report.html
    >>>

    output {
        File summary_report = sname + "_summary_report.html"
    }
