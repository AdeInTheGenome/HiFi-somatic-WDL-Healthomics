...
workflow align_all_bams {
    ...
    input {
        String sample
        Array[File] sample_tumor_bam_files
        Array[File] sample_normal_bam_files
        File ref_fasta
     ...
     if(!skip_align){
      scatter (tumor_bam in sample_tumor_bam_files) {

        call Align as TumorAlign {
          input:
            sample_name = sample + ".tumor",
            bam_file = tumor_bam,
            ref_fasta = ref_fasta,
      ...
      scatter (normal_bam in sample_normal_bam_files) {
        call Align as NormalAlign {
          input:
            sample_name = sample + ".normal",
            bam_file = normal_bam,
            ref_fasta = ref_fasta,
            ref_fasta_index = ref_fasta_index,
            additional_args = additional_pbmm2_args,
    ...
    # If more than one BAM, merge them
      if (!skip_align && length(sample_tumor_bam_files) > 1) {
      ...
      if (!skip_align && length(sample_normal_bam_files) > 1) {
      ...
      if (skip_align && length(sample_tumor_bam_files) > 1) {
        call MergeBams as MergeTumorSkipAlignBams {
          input:
            sample_name = sample + ".tumor",
            bam_files = sample_tumor_bam_files,
            threads = merge_bam_threads
        }
      }
      ...
      if (skip_align && length(sample_normal_bam_files) > 1) {
        call MergeBams as MergeNormalSkipAlignBams {
          input:
            sample_name = sample + ".normal",
            bam_files = sample_normal_bam_files,
            threads = merge_bam_threads
        }
      }
...
task Align {
  ...
  runtime {
    docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:pbmm2"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
