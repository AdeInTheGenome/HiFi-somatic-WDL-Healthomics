...
task truvari_filter {
  ...
  runtime {
    docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:truvari"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task svpack_filter_annotated {
	...
	runtime {
		docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:svpack"
		cpu: 4
    memory: "16 GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task mosdepth {
  ...
  output {
    File output_bed = sname + ".regions.bed.gz"
    File output_bed_index = sname + ".regions.bed.gz.csi"
    File output_summary = sname + ".mosdepth.summary.txt"
  }

  runtime {
    docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:mosdepth"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task cpg_pileup {
  ...
  String output_prefix = sname + ".cpg"
  Float file_size = ceil(size(bam, "GB") * 2)

  command <<<
  set -euxo pipefail

  echo "Running cpg pileup for ~{sname}"
  ...
  runtime {
    docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:pb-cpg-tools"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task strip_kinetics {
  ...
  runtime {
    docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:samtools"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task seqkit_bamstats {
  ...
  runtime {
    docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:seqkit"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task summarize_seqkit_alignment {
  ...
  runtime {
    docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:csvtk"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task mutationalpattern {
  ...
  output {
    File mutsig_tsv = sname + ".mut_sigs.tsv"
    File recon_sig = sname + ".reconstructed_sigs.tsv"
    File occurences_tsv = sname + ".type_occurences.tsv"
    File mutsig_bootstrap = sname + ".mut_sigs_bootstrapped.tsv"
    File mut_profile = sname + ".mutation_profile.pdf"
