...
task call_DeepSomatic {
    ...
    /opt/deepvariant/bin/deepsomatic/run_deepsomatic \
        ...
        --output_vcf=deepvariant_~{sname}/~{sname + "_deepsomatic.vcf.gz"} \
        --output_gvcf=deepvariant_~{sname}/~{sname + "_deepsomatic.g.vcf.gz"} \
        --sample_name_tumor=~{sname + ".tumor"} \
        --sample_name_normal=~{sname + ".normal"} \
        --num_shards=~{threads} \
        --logging_dir=deepvariant_~{sname}/logs \
        ~{"--regions=" + contig}
    ...
    bcftools view \
        -f PASS -Oz \
        -o deepvariant_~{sname}/~{sname + "_deepsomatic.PASS.vcf.gz"} \
        deepvariant_~{sname}/~{sname + "_deepsomatic.vcf.gz"}
    tabix -p vcf deepvariant_~{sname}/~{sname + "_deepsomatic.PASS.vcf.gz"}
    >>>

    output {
        File deepsomatic_vcf = "deepvariant_" + sname + "/" + sname + "_deepsomatic.PASS.vcf.gz"
        File deepsomatic_vcf_tbi = "deepvariant_" + sname + "/" + sname + "_deepsomatic.PASS.vcf.gz.tbi"
        File deepsomatic_gvcf = "deepvariant_" + sname + "/" + sname + "_deepsomatic.g.vcf.gz"
        File deepsomatic_gvcf_tbi = "deepvariant_" + sname + "/" + sname + "_deepsomatic.g.vcf.gz.tbi"
    }

    runtime {
        docker: "860660336427.dkr.ecr.us-east-1.amazonaws.com/hifisomatic:deepsomatic"
        cpu: threads
        memory: "~{threads * 6} GB"
        disk: file_size + " GB"
        maxRetries: 2
...
task gather_deepsomatic {
    ...
    echo "Merging deepsomatic vcfs for ~{sname}"
    ...
    bcftools concat -a -f vcf.list |\
        bcftools sort -Oz -o ~{sname + "_deepsomatic.vcf.gz"}
    bcftools concat -a -f gvcf.list |\
        bcftools sort -Oz -o ~{sname + "_deepsomatic.g.vcf.gz"}

    tabix -p vcf ~{sname + "_deepsomatic.vcf.gz"}
    tabix -p vcf ~{sname + "_deepsomatic.g.vcf.gz"}
    ...
    output {
        File deepsomatic_vcf = sname + "_deepsomatic.vcf.gz"
        File deepsomatic_vcf_tbi = sname + "_deepsomatic.vcf.gz.tbi"
        File deepsomatic_gvcf = sname + "_deepsomatic.g.vcf.gz"
        File deepsomatic_gvcf_tbi = sname + "_deepsomatic.g.vcf.gz.tbi"
    }
...
task call_clair3 {
    ...
    /opt/bin/run_clair3.sh \
        ...
        --output=clair3_~{sname} \
        --sample_name=~{sname}
    
    # Filter for PASS only
    gunzip -c clair3_~{sname}/merge_output.vcf.gz |\
        awk '{if($7!="LowQual"){print $0}}' OFS=$'\t' |\
        bgzip -c > ~{sname}.clair3.small_variants.vcf.gz
    tabix -p vcf ~{sname}.clair3.small_variants.vcf.gz
    >>>

    output {
        File clair3_vcf = sname + ".clair3.small_variants.vcf.gz"
        File clair3_vcf_tbi = sname + ".clair3.small_variants.vcf.gz.tbi"
    }
